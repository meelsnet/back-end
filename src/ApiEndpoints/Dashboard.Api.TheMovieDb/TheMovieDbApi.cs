...
using Dashboard.Settings.Settings.Models.External;
using Nito.AsyncEx;

namespace Dashboard.Api.TheMovieDb
{
    public class TheMovieDbApi : IMovieDbApi
    {
        public TheMovieDbApi(IMapper mapper, IApi api, ISettingsService<TheMovieDbSettings> settings)
        {
            Api = api;
            Mapper = mapper;
            Settings = new AsyncLazy<TheMovieDbSettings>(() => settings.GetSettingsAsync());
        }

        private const string ApiToken = "8e8b06b1cb21b2d3f36f8bd44c933672";
        ...
        /// </remarks>
        public async Task<List<MovieSearchResult>> PopularMovies(string langCode, int? page = null, CancellationToken cancellationToken = default)
        {
            var request = new Request($"discover/movie", BaseUri, HttpMethod.Get);
            request.AddQueryString("api_key", ApiToken);
            request.AddQueryString("language", langCode);
